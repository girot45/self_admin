<template>
  ...
  <div class="flex gap-5 mb-5">
    ...
    <div class="custom-card grow">
      ...
      <p
        ...
        :class="totalBudgetLeft >= 0 ? 'text-teal-600' : 'text-red-500'"
      >
      ...
      <template #content>
        <div class="flex justify-between items-start mb-3">
          <div class="flex items-center gap-3">
            <div class="cat-icon" :style="{ background: b.iconBg }">
              {{ b.emoji }}
            </div>
            <div>
        ...
        <div class="flex justify-between text-sm mb-1">
          ...
          <span
            :class="
              b.spent > b.limit ? 'text-red-500 font-semibold' : 'font-semibold'
            "
          >
            ₽{{ b.spent.toLocaleString("ru-RU") }} / ₽{{
              b.limit.toLocaleString("ru-RU")
            }}
          </span>
        </div>
        <ProgressBar
        ...
        >
          <template #value>
            <div
              ...
              :style="{ background: getBudgetColor(b) }"
            />
          </template>
        </ProgressBar>
  ...
  <ModalDialog
    ...
    :title="editingBudget?.id ? 'Редактировать бюджет' : 'Новый бюджет'"
    :confirm-label="editingBudget?.id ? 'Сохранить' : 'Создать'"
    :confirm-disabled="!bf.name || !bf.limit"
    @confirm="saveBudget"
  >
...
const totalBudgetLeft = computed(
  () => totalBudgetLimit.value - totalBudgetSpent.value,
);
